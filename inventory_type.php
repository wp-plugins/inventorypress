...
//add myInventory custom post templates
function my_inventory_post() {

	$my_inventory_page_id = get_option('INV_myInventory_page');
	$my_inventory_post = array();
	$my_inventory_post['post_title'] = 'myInventory';
	$my_inventory_post['post_content'] = 'This is a fake page used for the page "myInventory.php". When the plugin is installed properly this content is not displayed on the front-end page. This plugin requires the use of a "post name" permalink structure to ensure that myInventory page will be located in the proper url format, and also so that the page template filter can see that the myInventory page is being loaded.
The page filter allows the plugin to override this "myInventory" placeholder page and your themes default page template. If you need to use another permalink structure, it can be done but it would require some minor changes to the code in order to make it all work. In due time I'll create an options page for selecting other types of permalink structures. For now reset your permalinks';
 	$my_inventory_post['post_status'] = 'publish';
	$my_inventory_post['post_author'] = '1';
	$my_inventory_post['post_type'] = 'page';
	$my_inventory_post = wp_insert_post($my_inventory_post);
	add_option('INV_myInventory_page', $my_inventory_page_id);
}

register_activation_hook( __FILE__, 'my_inventory_post');

//add myInventory custom post template

function get_myinventory_template($page_template) {
     global $post;

     if ($post->post_name == 'myinventory') {
          $page_template = dirname( __FILE__ ) . '/themefiles/myInventory.php';
     }
     return $page_template;
}

add_filter( "page_template", "get_myinventory_template" ) ;

//add single inventory custom post templates

function get_inventory_template($single_template) {
     global $post;

     if ($post->post_type == 'inventory') {
          $single_template = dirname( __FILE__ ) . '/themefiles/single-inventory.php';
     }
     return $single_template;
}

add_filter( "single_template", "get_inventory_template" ) ;
...
<label>Description:</label><textarea name="inventory_item_desc" style="width:100%; min-height:150px;"><?php echo $inventory_item_desc; ?></textarea>
